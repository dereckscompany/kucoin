...
* **`ms_to_datetime()` / `ns_to_datetime()` no longer emit spurious `"NAs introduced by coercion"` warnings** when given an all-`NA_character_` vector. The NA → NA path is the documented contract, not a problem worth a warning. Wrapped `as.numeric()` in `suppressWarnings()` so genuine bad input still fails loudly (it just no longer surfaces the redundant warning on every all-NA column). Applies to every endpoint whose payload sometimes omits a timestamp field.
