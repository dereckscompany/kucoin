...
| `kucoin_build_request()` | Low-level HTTP / signing primitive — every method goes through this |
| `kucoin_paginate()` | Generic page-walker for paginated endpoints |
...
`kucoin_build_request()` constructs and signs every request, and
`kucoin_paginate()` walks the `currentPage` / `pageSize` envelope on
paginated endpoints. You should rarely call either directly — every
method goes through them — but both are exported so you can wrap any
...
- **No reconnect / retry on transient network errors.** The single
  ...
  through `kucoin_build_request()`) if you want retries.
