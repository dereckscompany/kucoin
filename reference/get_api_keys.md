...
- api_key:
  ...
  (scalar\<character\>) KuCoin API key. Defaults to
  `Sys.getenv("KUCOIN_API_KEY")`.

- api_secret:

  (scalar\<character\>) KuCoin API secret. Defaults to
  `Sys.getenv("KUCOIN_API_SECRET")`.

- api_passphrase:

  (scalar\<character\>) KuCoin API passphrase. Defaults to
  `Sys.getenv("KUCOIN_API_PASSPHRASE")`.

- key_version:

  (scalar\<character\>) API key version. Defaults to `"2"`.

## Value

(list) named list with `api_key`, `api_secret`, `api_passphrase`,
`key_version`.
